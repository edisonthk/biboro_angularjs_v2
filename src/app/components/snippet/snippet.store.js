class SnippetStore {
    ...
    fetchAll() {
        ...
        self._http.get('./fjoafja.json')
            .success(function(data){
                self.snippets.push(data);
                self._dispatcher.dispatch(this.SNIPPET_FETCHEDALL_CALLBACK, {"success":true,"result":"success","data":self.getSnippets()});
            })
            .error(function(){
                console.log("error in snippet.strore.js");
                self._dispatcher.dispatch(this.SNIPPET_FETCHEDALL_CALLBACK, {"success":false,"result":"fail to fetch snippets."});
            });
